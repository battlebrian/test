...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        for j in range(len(features)):
            error = 0
            curclf = WeakClassifier(features[j])
            for i in range(len(iis)):
                pred = curclf.classify(iis[i])
                error += weights[i] * (labels[i] != pred)
            if (error < bestError):
                bestClf = curclf
                bestError = error
        ...
        #     tmp.clear()
        #     for i in range(len(iis)):
        #         if (featureVals[j][i] < 0): tmp.append(1)
        #         else: tmp.append(0)
        #     pre.append(tmp)
        # for j in range(len(features)):
        #     error = 0
        #     for i in range(len(iis)):
        #         if (labels[i] == pre[j][i]): error += weights[i]
        #     if (error < bestError):
        #         curclf = j
        #         bestError = error
        # bestClf = WeakClassifier(features[curclf], 0, 1)
